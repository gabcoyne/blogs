...
cwlogs = cwlogs
[default]
region = ap-southeast-1" >> /etc/awslogs/awscli.conf
...
# get API key
result=$(aws secretsmanager get-secret-value --secret-id prefect_api_key --region ap-southeast-1)
secret=$(echo $result | jq -r '.SecretString')
PREFECT_API_KEY=$(echo $secret | jq -r '.prefect_api_key')
...
RestartSec=1
User=root
ExecStart=/usr/local/bin/prefect agent docker start -k $PREFECT_API_KEY --agent-address http://localhost:8080
[Install]
